...
URLParseName = function() {
        let URL = location.search;
        let query = URL.substring(6,URL.indexOf('&'));
	console.log(query);
        return query;
}
...
function sendAJAXStock() {
        ...
        xmlhttp.open('GET','http://34.138.248.85:80/search?company='+URLParseName()); 
        xmlhttp.send();
}

// function that grabs prices of a stock from database
function sendAJAXPrice() {
        ...
        // requesting data from server based on GET method queary
        xmlhttp.open('GET','http://34.138.248.85:80/price?company='+URLParseName());
        xmlhttp.send();
}

// function that saves page stock to user saved list
function sendAJAXSave() {
        ...
        // requesting data from server based on GET method queary
        xmlhttp.open('GET','http://34.138.248.85:80/save?user='+URLParseUser()+'&stock='+URLParseName());
        xmlhttp.send();
}
...
window.onload = function() {
        document.getElementById('title').innerHTML = 'Generic Stock - ' + URLParseName();
        document.getElementById('name').innerHTML = URLParseName();
...
// event listner for save stock button
document.getElementById('save').addEventListener('click', sendAJAXSave);
